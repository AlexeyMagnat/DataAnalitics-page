...
gulp.task('clean:dev', function (done) {
	if (fs.existsSync('./build/')) {
		return gulp
			.src('./build/', { read: false })
			.pipe(clean({ force: true }))
			.on('end', done); // Убедитесь, что done вызывается в любом случае
	}
	done();
});
...
// });
gulp.task('images:dev', function () {
	const imageFilter = filter(['**/*.png', '**/*.jpg', '**/*.jpeg'], { restore: true });
	...
	return gulp
		...
		.pipe(gulp.dest('./build/img/'))
		// .pipe(imagemin({ verbose: true })) // Если нужно включить
		.pipe(gulp.src(['./src/img/**/*', '!./src/img/svgicons/**/*'])) // Лишний вызов src убран
		.pipe(changed('./build/img/'))
		.pipe(gulp.dest('./build/img/'));
});
